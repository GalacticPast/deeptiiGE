...
static texture_system_state *texture_state_ptr = 0;
...
b8 texture_system_initialize(u64 *memory_requirement, void *state, texture_system_config config)
{
    ...
    *memory_requirement       = struct_requirement + array_requirement + hashtable_requirement;
    ...
    texture_state_ptr         = state;
    texture_state_ptr->config = config;

    // The array block is after the state. Already allocated, so just set the pointer.
    void *array_block                      = state + struct_requirement;
    texture_state_ptr->registered_textures = array_block;

    // Hashtable block is after array.
    void *hashtable_block = array_block + array_requirement;

    // Create a hashtable for texture lookups.
    hashtable_create(sizeof(texture_reference), config.max_texture_count, hashtable_block, false, &texture_state_ptr->registered_texture_table);
    ...
    hashtable_fill(&texture_state_ptr->registered_texture_table, &invalid_ref);

    // Invalidate all textures in the array.
    u32 count = texture_state_ptr->config.max_texture_count;
    for (u32 i = 0; i < count; ++i)
    {
        texture_state_ptr->registered_textures[i].id         = INVALID_ID;
        texture_state_ptr->registered_textures[i].generation = INVALID_ID;
    }

    // Create default textures for use in the system.
    create_default_textures(texture_state_ptr);
...
{
    if (texture_state_ptr)
    {
        // Destroy all loaded textures.
        for (u32 i = 0; i < texture_state_ptr->config.max_texture_count; ++i)
        {
            texture *t = &texture_state_ptr->registered_textures[i];
        ...
        destroy_default_textures(texture_state_ptr);
...
texture *texture_system_acquire(const char *name, b8 auto_release)
{
    ...
    if (strings_equali(name, DEFAULT_TEXTURE_NAME))
    {
        ...
        return &texture_state_ptr->default_texture;
    }

    texture_reference ref;

    b8 result = hashtable_get(&texture_state_ptr->registered_texture_table, name, &ref);

    if (texture_state_ptr && result)
    {
        ...
        if (ref.handle == INVALID_ID)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                if (texture_state_ptr->registered_textures[i].id == INVALID_ID)
                {
                    // A free slot has been found. Use its index as the handle.
                    ref.handle = i;
                    t          = &texture_state_ptr->registered_textures[i];
                    break;
                }
            }
        ...
        // Update the entry.
        hashtable_set(&texture_state_ptr->registered_texture_table, name, &ref);
        return &texture_state_ptr->registered_textures[ref.handle];
    }
...
void texture_system_release(const char *name)
{
    ...
    texture_reference ref;
    if (texture_state_ptr && hashtable_get(&texture_state_ptr->registered_texture_table, name, &ref))
    {
        if (ref.reference_count == 0)
        {
        ...
        {
            texture *t = &texture_state_ptr->registered_textures[ref.handle];
        ...
        // Update the entry.
        hashtable_set(&texture_state_ptr->registered_texture_table, name_copy, &ref);
    }
    else
    {
...
b8 create_default_textures(texture_system_state *state)
{
    ...
    renderer_create_texture(&state->default_texture, pixels);
...
b8 load_texture(const char *texture_name, texture *t)
{
    ...
    if (data)
    {
        ...
        renderer_create_texture(&temp_texture, data);
