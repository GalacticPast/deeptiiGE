...
static material_system_state *material_state_ptr = 0;
...
b8 material_system_initialize(u64 *memory_requirement, void *state, material_system_config config)
{
    ...
    *memory_requirement       = struct_requirement + array_requirement + hashtable_requirement;
    ...
    material_state_ptr         = state;
    material_state_ptr->config = config;

    // The array block is after the state. Already allocated, so just set the pointer.
    void *array_block                        = state + struct_requirement;
    material_state_ptr->registered_materials = array_block;

    // Hashtable block is after array.
    void *hashtable_block = array_block + array_requirement;

    // Create a hashtable for material lookups.
    hashtable_create(sizeof(material_reference), config.max_material_count, hashtable_block, false, &material_state_ptr->registered_material_table);
    ...
    hashtable_fill(&material_state_ptr->registered_material_table, &invalid_ref);

    // Invalidate all materials in the array.
    u32 count = material_state_ptr->config.max_material_count;
    for (u32 i = 0; i < count; ++i)
    {
        material_state_ptr->registered_materials[i].id          = INVALID_ID;
        material_state_ptr->registered_materials[i].generation  = INVALID_ID;
        material_state_ptr->registered_materials[i].internal_id = INVALID_ID;
    }

    if (!create_default_material(material_state_ptr))
    {
...
material *material_system_acquire_from_config(material_config config)
{
    ...
    {
        return &material_state_ptr->default_material;
    }

    material_reference ref;
    if (material_state_ptr && hashtable_get(&material_state_ptr->registered_material_table, config.name, &ref))
    {
        ...
        if (ref.handle == INVALID_ID)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                if (material_state_ptr->registered_materials[i].id == INVALID_ID)
                {
                    // A free slot has been found. Use its index as the handle.
                    ref.handle = i;
                    m          = &material_state_ptr->registered_materials[i];
                    break;
                }
            }
        ...
        // Update the entry.
        hashtable_set(&material_state_ptr->registered_material_table, config.name, &ref);
        return &material_state_ptr->registered_materials[ref.handle];
    }
...
void material_system_release(const char *name)
{
    ...
    material_reference ref;
    if (material_state_ptr && hashtable_get(&material_state_ptr->registered_material_table, name, &ref))
    {
        if (ref.reference_count == 0)
        {
        ...
        {
            material *m = &material_state_ptr->registered_materials[ref.handle];
        ...
        // Update the entry.
        hashtable_set(&material_state_ptr->registered_material_table, name, &ref);
    }
    else
    {
...
b8 load_material(material_config config, material *m)
{
    dzero_memory(m, sizeof(material));
    ...
    {
        // NOTE: Only set for clarity, as call to dzero_memory above does this already.
...
b8 create_default_material(material_system_state *state)
{
    dzero_memory(&state->default_material, sizeof(material));
...
b8 load_configuration_file(const char *path, material_config *out_config)
{
    ...
    {
        DERROR("load_configuration_file - unable to open material file for reading: '%s'.", path);
        return false;
    }
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        if (equal_index == -1)
        {
            DWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui.", path, line_number);
            line_number++;
            continue;
        }
        ...
        else if (strings_equali(trimmed_var_name, "diffuse_color"))
        {
            ...
            {
                DWARN("Error parsing diffuse_color in file '%s'. Using default of white instead.", path);
        ...
        dzero_memory(line_buf, sizeof(char) * 512);
        line_number++;
    }
